...
In the example below, we load the sample BBO data into a list called `accum` twice, similar to how we might bind multiple days of data together. From a memory use perspective it is good to get used to storing intermediate data in lists and then binding them together in the end. For more discussion of memory use problems in R see the excellent post at [win-vector](http://www.win-vector.com/blog/2015/07/efficient-accumulation-in-r/). Then, the function `rbindlist()` from the `data.table` package binds the two elements of the list, `accum`, into one data.table called `corn_110110`. The next line, `corn_110110[, Price := decimalprices(corn_110110$TrPrice)]` exploits the `data.table` functionality of calling a function `decimalprices()` on a column of the data.table and creating a new column called `Price`. More efficient from a memory perspective would be to simply replace the `TrPrice` columns with decimal prices returned by `decimalprices()`. Then the line would be `corn_110110[, TrPrice := decimalprices(corn_110110$TrPrice)]` instead. 
...
```{r}
corn_110110[, bins := minutebins(corn_110110, 10)]
corn_110110
...
corn_110110[, c("TradeDate", "TradeTime") := .(datemanip(TradeDate), timemanip(TradeTime))]
corn_110110
...
A frequent topic of analysis with the BBO data is to 

```{r}
DT <- dcast.data.table(corn_110110[ ASKBID != 'NA'  ], 
                       TradeDate + TradeTime + TradeSeqNum + DeliveryDate + EntryDate ~ ASKBID, 
                       value.var = c("TrQuantity", "TrPrice"))
